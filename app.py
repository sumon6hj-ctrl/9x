...
ADMIN_PASSWORD = os.environ.get("ADMIN_PASSWORD", "sumon123")
...
# ─── START SERVER (Direct - no JSON response) ──────────────────────────────
...
# ─── STOP SERVER (Direct - no JSON response) ───────────────────────────────
    ...
    try:
        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
            try:
                ...
                if name in cmdline and proc.pid != pid:
                    proc.terminate()
                    try:
                        proc.wait(timeout=2)
                    except Exception:
